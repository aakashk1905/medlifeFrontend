...
{content?.SurgeryForm?.surgeryTypeHeader && (
  <h1 className="text-2xl font-bold mb-3 text-gray-600">
    {content?.SurgeryForm?.surgeryTypeHeader}
        ...
        return (
          <p className="text-sm mb-2 font-lato font-normal text-justify" key={index}>
...
<div className="grid grid-cols-1 md:grid-cols-2">
  {content?.SurgeryForm?.typeHeader && (
    <p className="text-sm text-gray-800">
      <span className="font-bold">{content?.SurgeryForm?.typeHeader}</span> - {content?.SurgeryForm?.type}
    </p>
  )}

  {content?.SurgeryForm?.surgeryNameHeader && (
    <p className="text-sm text-gray-800">
      <span className="font-bold">{content?.SurgeryForm?.surgeryNameHeader}</span> - {content?.SurgeryForm?.surgeryName}
    </p>
  )}

  {content?.SurgeryForm?.timeHeader && content?.SurgeryForm?.time && (
    <p className="text-sm text-gray-800">
      <span className="font-bold">{content?.SurgeryForm?.timeHeader}</span> - {content?.SurgeryForm?.time}
    </p>
  )}

  {content?.SurgeryForm?.costHeader && content?.SurgeryForm?.cost && (
    <p className="text-sm text-gray-800">
      <span className="font-bold">{content?.SurgeryForm?.costHeader}</span> - {content?.SurgeryForm?.cost}
    </p>
  )}

  {content?.SurgeryForm?.footerTitle && (
    <p className="text-sm text-gray-800">{content?.SurgeryForm?.footerTitle}</p>
  )}

  {content?.SurgeryForm?.formTitle && (
    <p className="text-sm text-gray-800">{content?.SurgeryForm?.formTitle}</p>
  )}
</div>
...
{content?.SurgeryForm?.treatmentOptionDetails?.map((option, index) => {
